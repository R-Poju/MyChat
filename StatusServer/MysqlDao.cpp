...
//        pstmt->setString(1, email);//将username替换为要查询的用户名
//
//        //执行查询
//        std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
...
bool MysqlDao::CheckPwd(const std::string& email, const std::string& pwd, UserInfo& userInfo) {
    ...
    try {
        ...
        // 执行查询
        std::cout << "Query Email: " << email << std::endl; // 检查传入的email是否正确
        std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
        std::string origin_pwd = "";
        // 遍历结果集
        while (res->next()) {
            ...
            origin_pwd = res->getString("pwd");
            std::cout << "DB Email: " << db_email
                << ", DB Pwd: " << origin_pwd << std::endl; // 输出数据库中的实际值
